...
private:
    ...
    void identifier(std::vector<Token>& tokens) {
        ...
        static const std::unordered_map<std::string, TokenType> keywords = {
            ...
            {"import", TokenType::IMPORT}, {"as", TokenType::AS}, // <-- 新增
...
struct IncludeStmt final : Stmt {
    Token path; // The string literal token containing the file path
    IncludeStmt(Token p, int ln) : Stmt(ln), path(std::move(p)) {}
};
// 在 IncludeStmt 定义之后添加
struct ImportStmt final : Stmt {
    Token path;
    Token alias;
    ImportStmt(Token p, Token a, int ln) : Stmt(ln), path(std::move(p)), alias(std::move(a)) {}
};
...
private:
    ...
    StmtPtr parseImportStatement(); // <-- 新增
...
StmtPtr Parser::parseIncludeStatement() {
    int ln = previous().line;
    Token path = consume(TokenType::STR, "Expect file path string after 'include'.");
    consume(TokenType::SEMICOLON, "Expect ';' after include statement.");
    return std::make_unique<IncludeStmt>(std::move(path), ln);
}
// =================================================================================
// ================================ FIXED FUNCTION =================================
// =================================================================================
StmtPtr Parser::parseTryStatement() {
    int ln = previous().line; // 'try' token's line number
    ...
    // Construct and return the TryStmt AST node
...
StmtPtr Parser::parseImportStatement() {
    int ln = previous().line;
    Token path = consume(TokenType::STR, "Expect file path string after 'import'.");
    consume(TokenType::AS, "Expect 'as' keyword after import path.");
    Token alias = consume(TokenType::ID, "Expect module alias name after 'as'.");
    consume(TokenType::SEMICOLON, "Expect ';' after import statement.");
    return std::make_unique<ImportStmt>(std::move(path), std::move(alias), ln);
}
// =================================================================================
// =================================================================================
// =================================================================================
...
std::string Value::toString() const { return std::visit(make_overloaded([](NullType) -> std::string { return std::string("nil"); }, [](int v) -> std::string { return std::to_string(v); }, [](double v) -> std::string { std::ostringstream oss; oss << v; return oss.str(); }, [](bool v) -> std::string { return v ? std::string("true") : std::string("false"); }, [](const std::string& v) -> std::string { return v; }, [](const FuncType& v) -> std::string { return v ? v->toString() : std::string("<null function>"); }, [this](const ArrayType& v) -> std::string { std::string result = "["; if (v) { for (size_t i = 0; i < v->size(); ++i) { if (i > 0) result += ", "; result += (*v)[i].toString(); } } return result + "]"; }, [this](const DictType& v) -> std::string { std::string result = "{"; if (v) { bool first = true; for (const auto& pair : *v) { if (!first) result += ", "; result += "\"" + pair.first + "\": "; result += pair.second.toString(); first = false; } } return result + "}"; }, [](const ObjectType& v){ if (!v) return std::string("nil"); auto it = v->_fields.find("__repr__"); if(it != v->_fields.end() && it->second.is<FuncType>()){ return it->second.as<FuncType>()->call({Value(v)}).toString(); } return "<" + v->_type_name() + " instance>"; }), data); }
...
Value _op_index_get(const Value& obj, const Value& idx) {
    ...
    if (obj.is<Value::ObjectType>()) { const auto& inst = obj.as<Value::ObjectType>(); auto it = inst->_fields.find(idx.toString()); if(it != inst->_fields.end()) return it->second; }
...
// --- Global State & Module System ---
static std::shared_ptr<std::unordered_map<std::string, Value>> _global_env;
static std::unordered_map<std::string, Value> _module_cache;
Value _cached_module_loader(const std::string& path, const std::function<Value()>& initializer) {
    if (_module_cache.count(path)) {
        return _module_cache.at(path);
    }
    Value module_obj = initializer();
    _module_cache[path] = module_obj;
    return module_obj;
}
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char* argv[]) {
    _global_env = std::make_shared<std::unordered_map<std::string, Value>>();
    try {
        (*_global_env)["nil"] = _V_NULL;
        // Core functions
        ...
        (*_global_env)["input"] = Value(std::make_shared<NativeFunction>([](const std::vector<Value>& args) -> Value {if (args.size() > 1) {throw std::runtime_error("input() takes 0 or 1 arguments.");}if (args.size() == 1) {std::cout << args[0].toString();std::cout.flush(); }std::string line;if (!std::getline(std::cin, line)) {return _V_NULL;}return Value(line);}, -1, "input"));
        // File & System
        (*_global_env)["clock"] = Value(std::make_shared<NativeFunction>([]([[maybe_unused]] const std::vector<Value>& args) -> Value { auto now = std::chrono::high_resolution_clock::now(); auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(now - _start_time).count(); return Value(static_cast<int>(duration)); }, 0, "clock"));
        (*_global_env)["read_file"] = Value(std::make_shared<NativeFunction>([](const std::vector<Value>& args) -> Value { if (!args[0].is<std::string>()) throw std::runtime_error("Argument to read_file must be a string path."); const std::string& path = args[0].as<std::string>(); std::ifstream file(path); if (!file.is_open()) throw std::runtime_error("Failed to open file for reading: " + path); std::stringstream buffer; buffer << file.rdbuf(); return Value(buffer.str()); }, 1, "read_file"));
        (*_global_env)["write_file"] = Value(std::make_shared<NativeFunction>([](const std::vector<Value>& args) -> Value { if (!args[0].is<std::string>()) throw std::runtime_error("First argument to write_file must be a string path."); if (!args[1].is<std::string>()) throw std::runtime_error("Second argument to write_file must be a string content."); const std::string& path = args[0].as<std::string>(); const std::string& content = args[1].as<std::string>(); std::ofstream file(path); if (!file.is_open()) throw std::runtime_error("Failed to open file for writing: " + path); file << content; return _V_NULL; }, 2, "write_file"));
        {
// MAIN_CODE_GOES_HERE
        }
...
class Transpiler {
    enum class VarStorage {
        Stack,
        HeapPtr,
        GlobalMap,
        ModuleMap // <-- 新增: 表示变量存储在当前编译模块的导出表中
    };
    ...
    // 新增: 模块管理
    std::set<std::string> compiled_modules;
    std::stringstream ss_module_definitions;

    std::string compile(const Stmt& s, TypeMap& types, bool is_in_function_scope, std::set<std::string>& include_guard, bool is_compiling_module, bool is_method = false, const std::string& class_name = "");
    std::string compile(const Expr& e, TypeMap& types, bool is_in_function_scope, std::set<std::string>& include_guard, bool is_compiling_module, bool is_method = false, const std::string& class_name = "");
    std::string compile_function_body(const std::vector<ParamInfo>& params, const BlockStmt& body, const std::string& func_name, TypeMap& types, std::set<std::string>& include_guard, bool is_compiling_module, bool is_method, const std::string& class_name);
...
public:
    std::string transpile(const StmtList& ast) {
        ...
        for (const auto& s : ast) {
            if (!s) continue;
            VarStorage storage = VarStorage::GlobalMap;
            if (auto p = dynamic_cast<const VarDeclStmt*>(s.get())) {
                globalTypes[p->name] = {"Value", storage};
            } else if (auto p = dynamic_cast<const FuncStmt*>(s.get())) {
                globalTypes[p->name] = {"Function", storage};
            } else if (auto p = dynamic_cast<const ClassStmt*>(s.get())) {
                class_names.insert(p->name);
                class_nodes[p->name] = p;
                globalTypes[p->name] = {p->name, storage};
            } else if (auto p = dynamic_cast<const ImportStmt*>(s.get())) {
                 globalTypes[p->alias.lexeme] = {"Module", storage};
            }
        }
        ...
        for (const auto& s : ast) {
            if (s && dynamic_cast<const ClassStmt*>(s.get())) {
                ss_global << compile(*s, globalTypes, false, include_guard, false, false, "") << "\n";
            }
        }

        for (const auto& s : ast) {
            if(s && !dynamic_cast<const ClassStmt*>(s.get())) {
                ss_main << "        " << compile(*s, globalTypes, false, include_guard, false, false, "") << "\n";
            }
        }
        
        std::string final_code = CPP_PRELUDE;
        std::string all_definitions = ss_global.str() + "\n" + ss_module_definitions.str();
        auto placeholder_pos = final_code.find("// FORWARD_DECLARATIONS_AND_CLASS_DEFINITIONS");
        if(placeholder_pos != std::string::npos) {
            final_code.replace(placeholder_pos, strlen("// FORWARD_DECLARATIONS_AND_CLASS_DEFINITIONS"), all_definitions);
        }
...
private:
    std::string sanitize(const std::string& n) {
        std::string s = "_var_";
        for (char c : n) {
            if (std::isalnum(c)) s += c; else s += '_';
        }
        return s;
    }
     std::string sanitize_path_to_identifier(const std::string& path) {
        std::string s = "_module_";
        for (char c : path) {
            if (std::isalnum(c)) s += c; else s += '_';
        }
        return s;
...
std::string Transpiler::compile(const Stmt& s, TypeMap& types, bool is_in_function_scope, std::set<std::string>& include_guard, bool is_compiling_module, bool is_method, const std::string& class_name) {
    if (auto p = dynamic_cast<const ImportStmt*>(&s)) {
        const std::string& path = p->path.lexeme;
        const std::string& alias = p->alias.lexeme;
        std::string module_func_name = sanitize_path_to_identifier(path);

        if (compiled_modules.find(path) == compiled_modules.end()) {
            compiled_modules.insert(path);

            std::ifstream file(path);
            if (!file.is_open()) {
                throw std::runtime_error("Failed to open imported file: '" + path + "' at line " + std::to_string(p->line));
            }
            std::string module_source((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
            
            Lexer lexer(module_source);
            auto tokens = lexer.tokenize();
            Parser parser(std::move(tokens));
            StmtList module_ast = parser.parse();

            ss_module_definitions << "\n// Module initializer for " << path << "\n";
            ss_module_definitions << "Value " << module_func_name << "() {\n";
            ss_module_definitions << "    auto _module_exports = std::make_shared<std::unordered_map<std::string, Value>>();\n";
            
            TypeMap module_types;
            std::set<std::string> module_include_guard;
            for(const auto& stmt : module_ast) {
                if(!stmt) continue;
                 if (auto vds = dynamic_cast<const VarDeclStmt*>(stmt.get())) {
                    module_types[vds->name] = {"Value", VarStorage::ModuleMap};
                } else if (auto fs = dynamic_cast<const FuncStmt*>(stmt.get())) {
                    module_types[fs->name] = {"Function", VarStorage::ModuleMap};
                }
            }
            
            for (const auto& stmt : module_ast) {
                if(stmt) {
                    ss_module_definitions << "    " << compile(*stmt, module_types, false, module_include_guard, true, false, "") << "\n";
                }
            }

            ss_module_definitions << "    auto _module_obj = std::make_shared<Object>();\n";
            ss_module_definitions << "    _module_obj->_fields = *_module_exports;\n";
            ss_module_definitions << "    return Value(_module_obj);\n";
            ss_module_definitions << "}\n";
        }
        
        return "(*_global_env)[\"" + alias + "\"] = _cached_module_loader(\"" + path + "\", &" + module_func_name + ");";
    }

    if (auto p = dynamic_cast<const IncludeStmt*>(&s)) {
        const std::string& path = p->path.lexeme;
        if (include_guard.count(path)) {
            throw std::runtime_error("Circular include detected: file '" + path + "' is already being included.");
        }
        include_guard.insert(path);
        std::ifstream file(path);
        if (!file.is_open()) {
            throw std::runtime_error("Failed to open included file: '" + path + "' at line " + std::to_string(p->line));
        }
        std::string included_source((std::istreambuf_iterator<char>(file)), std::istreambuf_iterator<char>());
        Lexer lexer(included_source);
        auto tokens = lexer.tokenize();
        Parser parser(std::move(tokens));
        StmtList included_ast = parser.parse();
        std::stringstream ss;
        for (const auto& stmt : included_ast) {
            if (stmt) {
                ss << compile(*stmt, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) << "\n";
            }
        }
        include_guard.erase(path);
        return ss.str();
    }
    if (auto p = dynamic_cast<const ExprStmt*>(&s)) {
        return compile(*p->expr, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) + ";";
    }
    if (auto p = dynamic_cast<const VarDeclStmt*>(&s)) {
        ...
        if (is_compiling_module) {
            types[p->name] = {var_type, VarStorage::ModuleMap};
            ss << "(*_module_exports)[\"" << p->name << "\"]";
        } else if (is_in_function_scope) {
            types[p->name] = {var_type, VarStorage::HeapPtr};
            ss << "auto " << sanitize(p->name) << " = std::make_shared<Value>(";
            if (p->initializer) {
                ss << compile(*p->initializer, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name);
            } else {
                ss << "_V_NULL";
            }
            ss << ");";
            return ss.str(); // Early return for this case
        } else {
            types[p->name] = {var_type, VarStorage::GlobalMap};
            ss << "(*_global_env)[\"" << p->name << "\"]";
        }
    ...
    if (auto p = dynamic_cast<const BlockStmt*>(&s)) {
        ...
        for (const auto& st : p->statements) {
            if (st) ss << "            " << compile(*st, blockTypes, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) << "\n";
        }
        ss << "        }";
        return ss.str();
    }
    if (auto p = dynamic_cast<const IfStmt*>(&s)) {
         std::string else_str = p->elseBranch ? " else " + compile(*p->elseBranch, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) : "";
         return "if ((" + compile(*p->condition, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) + ").toBool()) " + compile(*p->thenBranch, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) + else_str;
    }
    if (auto p = dynamic_cast<const WhileStmt*>(&s)) {
        return "while ((" + compile(*p->condition, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) + ").toBool()) " + compile(*p->body, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name);
    }
    if (auto p = dynamic_cast<const ForStmt*>(&s)) {
        ...
        ss << "{\n";
        if (p->initializer) {
            ss << "            " << compile(*p->initializer, for_scope_types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) << "\n";
        }
        ss << "            for (; "
           << (p->condition ? "(" + compile(*p->condition, for_scope_types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) + ").toBool()" : "true") << "; "
           << (p->increment ? compile(*p->increment, for_scope_types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) : "") << ") "
           << compile(*p->body, for_scope_types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) << "\n";
        ss << "        }";
        return ss.str();
    }
    if (auto p = dynamic_cast<const ForEachStmt*>(&s)) {
        ...
        body_scope_types[p->variableName] = {"Value", VarStorage::HeapPtr};
        ss << "{\n";
        ss << "            auto " << iter_var << " = " << compile(*p->iterable, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) << ";\n";
        ...
        ss << "                    " << compile(*p->body, body_scope_types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) << "\n";
        ss << "                }\n";
    ...
    if (auto p = dynamic_cast<const FuncStmt*>(&s)) {
        std::stringstream ss;
        if (is_compiling_module) {
            types[p->name] = {"Function", VarStorage::ModuleMap};
            ss << "(*_module_exports)[\"" << p->name << "\"] = " << compile_function_body(p->params, *p->body, p->name, types, include_guard, is_compiling_module, is_method, class_name) << ";";
        } else {
            types[p->name] = {"Function", VarStorage::GlobalMap};
            ss << "(*_global_env)[\"" << p->name << "\"] = " << compile_function_body(p->params, *p->body, p->name, types, include_guard, is_compiling_module, is_method, class_name) << ";";
        }
        return ss.str();
    }
    // ... (ClassStmt, ReturnStmt, etc. do not need changes for module compilation logic)
    if (auto p = dynamic_cast<const ClassStmt*>(&s)) {
        ...
        if (init_method) {
                        ...
                        if (auto* call = dynamic_cast<CallExpr*>(es->expr.get())) {
                            if (auto* super = dynamic_cast<SuperExpr*>(call->callee.get())) {
                                if (super->method.lexeme == "init") {
                                    ...
                                    for(const auto& param : init_method->params) temp_types[param.name] = {"Value", VarStorage::Stack};
                                    for (size_t i = 0; i < call->args.size(); ++i) {
                                        ss << compile(*call->args[i], temp_types, true, include_guard, is_compiling_module, true, p->name) << (i < call->args.size() - 1 ? ", " : "");
                                    }
                                    ss << ")";
                                }
                            }
            ...
            for(const auto& param : init_method->params) methodTypes[param.name] = {"Value", VarStorage::Stack};
            for (size_t i = (has_super_call ? 1 : 0); i < init_method->body->statements.size(); ++i) {
                ss << "        " << compile(*init_method->body->statements[i], methodTypes, true, include_guard, is_compiling_module, true, p->name) << "\n";
            }
            ss << "    }\n";
        } else {
             ss << "    _class_" << p->name << "() {}\n";
        ...
        for (const auto& method : p->methods) {
            ...
            ss << ") {\n";
            TypeMap methodTypes = types;
            for(const auto& param : method->params) methodTypes[param.name] = {"Value", VarStorage::Stack};
            ss << "        " << compile(*method->body, methodTypes, true, include_guard, is_compiling_module, true, p->name) << "\n";
            ss << "        return _V_NULL;\n";
            ss << "    }\n";
        }
        ss << "};";
    ...
    if (auto p = dynamic_cast<const ReturnStmt*>(&s)) {
        if (is_in_function_scope) {
            if (p->expr) {
                return "return " + compile(*p->expr, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) + ";";
            } else {
                return "return _V_NULL;";
            }
        } else {
            return "return 0;";
        }
    ...
    if (auto p = dynamic_cast<const ThrowStmt*>(&s)) {
        return "throw _ThrowSignal(" + compile(*p->expr, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) + ");";
    }
    if (auto p = dynamic_cast<const TryStmt*>(&s)) {
        std::stringstream ss;
        ss << "try " << compile(*p->try_block, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name);
        ss << " catch (const _ThrowSignal& _signal) ";
        TypeMap catch_types = types;
        catch_types[p->catch_variable.lexeme] = {"Value", VarStorage::HeapPtr};
        ss << "{\n";
        ss << "            auto " << sanitize(p->catch_variable.lexeme) << " = std::make_shared<Value>(_signal.thrown_value);\n";
        ss << "            " << compile(*p->catch_block, catch_types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) << "\n";
        ss << "        }";
        return ss.str();
    }
    
    return "// Unsupported Stmt\n";
}std::string Transpiler::compile(const Expr& e, TypeMap& types, bool is_in_function_scope, std::set<std::string>& include_guard, bool is_compiling_module, bool is_method, const std::string& class_name) {
    if (auto p = dynamic_cast<const LiteralExpr*>(&e)) {
        ...
        std::visit(make_overloaded(
            ...
            [&]([[maybe_unused]] const auto& unused){ ss << "Value()"; }
        ), p->value.getVariant());
        return ss.str();
    }
    if (auto p = dynamic_cast<const BinaryExpr*>(&e)) {
        std::string l_str = compile(*p->left, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name);
        std::string r_str = compile(*p->right, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name);
         switch (p->op.type) {
    ...
    if (auto p = dynamic_cast<const UnaryExpr*>(&e)) {
        return p->op.type == TokenType::MINUS
            ? "_op_unary_minus(" + compile(*p->expr, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) + ")"
            : "_op_not(" + compile(*p->expr, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) + ")";
    }
    if (auto p = dynamic_cast<const VarExpr*>(&e)) {
        if (types.count(p->name)) {
            switch(types.at(p->name).storage) {
                case VarStorage::Stack:     return sanitize(p->name);
                case VarStorage::HeapPtr:   return "(*" + sanitize(p->name) + ")";
                case VarStorage::GlobalMap: return "(*_global_env)[\"" + p->name + "\"]";
                case VarStorage::ModuleMap: return "(*_module_exports)[\"" + p->name + "\"]";
            }
        }
    ...
    if (auto p = dynamic_cast<const CallExpr*>(&e)) {
        ...
        if (auto* var_callee = dynamic_cast<const VarExpr*>(p->callee.get())) {
            if (class_names.count(var_callee->name)) {
                ...
                for (size_t i = 0; i < p->args.size(); ++i) {
                    ss << compile(*p->args[i], types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) << (i < p->args.size() - 1 ? ", " : "");
                }
                ss << "))";
                return ss.str();
            }
        ...
        if (auto* member_callee = dynamic_cast<const MemberAccessExpr*>(p->callee.get())) {
            std::string obj_code = compile(*member_callee->object, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name);
        ...
        ss << "_call(" << compile(*p->callee, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) << ", {";
        for (size_t i = 0; i < p->args.size(); ++i) {
            ss << compile(*p->args[i], types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) << (i < p->args.size() - 1 ? ", " : "");
        }
        ss << "})";
    ...
    if (auto p = dynamic_cast<const AssignExpr*>(&e)) {
        std::string value_str = compile(*p->value, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name);
        if (auto* var_target = dynamic_cast<const VarExpr*>(p->target.get())) {
             if (types.count(var_target->name)) {
                switch(types.at(var_target->name).storage) {
                    case VarStorage::Stack:
                        return "(" + sanitize(var_target->name) + " = " + value_str + ")";
                    case VarStorage::HeapPtr:
                        return "(*" + sanitize(var_target->name) + " = " + value_str + ")";
                    case VarStorage::GlobalMap:
                         return "((*_global_env)[\"" + var_target->name + "\"] = " + value_str + ")";
                    case VarStorage::ModuleMap:
                         return "((*_module_exports)[\"" + var_target->name + "\"] = " + value_str + ")";
                }
            }
            return "((*_global_env)[\"" + var_target->name + "\"] = " + value_str + ")";
        }
        if (auto* index_target = dynamic_cast<const IndexExpr*>(p->target.get())) {
            ...
            if (auto* var_obj = dynamic_cast<const VarExpr*>(index_target->object.get())) {
                 obj_str = compile(*var_obj, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name);
            } else {
                obj_str = compile(*index_target->object, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name);
            }
            std::string idx_str = compile(*index_target->index, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name);
        ...
        if (auto* member_target = dynamic_cast<const MemberAccessExpr*>(p->target.get())) {
            ...
            std::string obj_code = compile(*member_target->object, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name);
    ...
    if (auto p = dynamic_cast<const ArrayLiteralExpr*>(&e)) {
        ...
        for (size_t i = 0; i < p->elements.size(); ++i) {
            ss << compile(*p->elements[i], types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) << (i < p->elements.size() - 1 ? ", " : "");
        }
        ss << "}))";
        return ss.str();
    }
    if (auto p = dynamic_cast<const DictLiteralExpr*>(&e)) {
        ...
        for (size_t i = 0; i < p->pairs.size(); ++i) {
            ss << "{\"" << p->pairs[i].first << "\", " << compile(*p->pairs[i].second, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) << "}" << (i < p->pairs.size() - 1 ? ", " : "");
        }
        ss << "})))";
        return ss.str();
    }
    if (auto p = dynamic_cast<const IndexExpr*>(&e)) {
        return "_op_index_get(" + compile(*p->object, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) + ", " + compile(*p->index, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name) + ")";
    }
    if (auto p = dynamic_cast<const MemberAccessExpr*>(&e)) {
        ...
        std::string obj_code = compile(*p->object, types, is_in_function_scope, include_guard, is_compiling_module, is_method, class_name);
    ...
    if (auto p = dynamic_cast<const FuncLiteralExpr*>(&e)) {
        return compile_function_body(p->params, *p->body, "<lambda>", types, include_guard, is_compiling_module, is_method, class_name);
    }
    return "/* Unsupported Expr */";
}

std::string Transpiler::compile_function_body(const std::vector<ParamInfo>& params, const BlockStmt& body, const std::string& func_name, TypeMap& types, std::set<std::string>& include_guard, bool is_compiling_module, bool is_method, const std::string& class_name) {
    std::stringstream ss;
    ss << "Value(std::make_shared<UserFunction>(" << params.size() << ", \"" << func_name << "\", "
       << "[=](const std::vector<Value>& args) mutable -> Value {\n"; 
    
    auto func_include_guard = include_guard;

    TypeMap bodyTypes = types;
    for (size_t i = 0; i < params.size(); ++i) {
        bodyTypes[params[i].name] = {"Value", VarStorage::Stack};
        ss << "            Value " << sanitize(params[i].name) << " = args[" << i << "];\n";
    }
    
    // Always true inside a function scope
    ss << "        " << compile(body, bodyTypes, true, func_include_guard, is_compiling_module, is_method, class_name) << "\n";
    ss << "\n            return _V_NULL;\n        }))";
    return ss.str();
}
...
int main() {
    // --- Test File Setup ---
    const char* lib_filename = "math_lib.mylang";
    std::cout << "--- Test Setup: Creating '" << lib_filename << "' ---\n";
    std::ofstream lib_file(lib_filename);
    if (!lib_file) {
        std::cerr << "Failed to create test library file.\n";
        return 1;
    }
    lib_file << R"CODE(
// This is an independent math library module
var PI = 3.14159; // Module's internal PI

func circle_area(radius) {
    return PI * radius * radius;
}

print("Debug: math_lib.mylang has been executed.");
)CODE";
    lib_file.close();
    std::cout << "Test library file created successfully.\n\n";

    // Main source code that uses the import feature
    /*
...
var PI = "I am a delicious pie!"; // Main program's PI, should not conflict

print("Main program's PI is:", PI);
print("Attempting to import 'math_lib.mylang' as math...");

import "math_lib.mylang" as math;
import "math_lib.mylang" as math_again; // Should use cache, not execute again

print("\nModule imported successfully.");
print("The type of 'math' is:", type(math));
print("The type of 'math_again' is:", type(math_again));
assert(type(math) == "object", "Module should be an object");

print("\nAccessing members from the imported module:");
print("The value of PI from the library is:", math.PI);
assert(math.PI == 3.14159, "Module PI value is incorrect!");

var r = 10;
var area = math.circle_area(r);
print("Area of a circle with radius", r, "is:", area);
assert(area == 314.159, "Module function call failed!");

print("\nVerifying main scope was not polluted:");
print("The main program's PI is still:", PI);
assert(PI == "I am a delicious pie!", "Global scope was polluted!");

print("\nVERIFICATION PASSED: Import system is working correctly!");
